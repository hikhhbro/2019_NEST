...
#include <Math.h>
 ...
 /**
  ...
  int16_t xxx = 2190,yyy =1928;
  float temmmm=0;int16_t saf=0;
  int16_t dis_temp=0;
  float angle_temp = 0;
  float angle_temp2 = 0,angle_temp3=0;
  int16_t dell = 100,dell1= 400;
  int16_t dd = 200;
  uint8_t end_id_f = 255;
  float roll = 0;
  int16_t delay1=50,delay2=100,delay3=400;
  float en_angle = 0.07;
  ...
  float lastdd=0;
    ...
    switch(hmi_t.page)
    {
      ...
      case P2_P: //	定距离发射         
        ...
        //        if(hmi_t.commot==1)
        //        {
        //            DXL1_setSyncTarAng(&huart2,2,0x00,ex_angle,0x01,dis_temp);
        //            osDelay(300);
        //           ConfirmTheLaunch(&hmi_t);//确认发送
        //        }       
      ...
      case F1_P://指定任意位置发射
        if(hmi_t.commot ==1)  //启动
        {
          ...
          else  if(pc_t.shoot_commot ==0)
          {
            if(flag17==0)
            {
              if(angle_temp3>20)
                flag17=1;
              else angle_temp3 = angle_temp3+1;
            }
            if(flag17==1)
            {
              if(angle_temp3<(-20))
            ...
            angle_temp3 = GetAngle(angle_temp3);//获取角度
            dis_temp = GetDistance(pc_t.distance);//获取距离 
            if(angle_temp3 != ERROR_ANGLE)
            {
              lastdd = angle_temp3;
              SetReal(r_distance,angle_temp3);
              osDelay(20);
            }
            else
            {
      ...
      case F2_P://寻找目标发射            
        ...
        roll =  ((float)ex_distance/POLES )* 360.0f-169.45f;
        printf("n1.val=%d%c%c%c",pc_t.distance,end_id_f,end_id_f,end_id_f);
...
/**
 ...
 /**
	...
	* @retval  void
	*/
 float a1 =  2448.0, b1 =4853,c1 =6542,a2 =39.65, b2 =2052,c2 =131.9;
 void SetReal(int16_t dis ,float angle)
 {
   //距离转换舵机线数
   //float  dis_a=0;
    r_distance = dis;                                                                                     //    roll =  (ex_distance/POLES )* 360.0f-169.45f;
    ex_distance = (int16_t)(a1*exp(-pow(((r_distance-b1)/c1),2)) + a2*exp(-pow(((r_distance-b2)/c2),2)));//;       a1*exp(-((x-b1)/c1)^2) + a2*exp(-((x-b2)/c2)^2)
    //角度转换舵机线数
    r_angle = angle;  
    ex_angle = (int16_t)(((angle+192.48f) / 360.0f) * POLES);
